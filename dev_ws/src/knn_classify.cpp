...
void KnnClassifier::Train(const std::string& folderPath) {
    ...
    for (const auto& imagePath : imagePaths) {
        ...
        if (image.empty()) {
            std::cout << imagePath << " can't read this image" << std::endl;
            continue;
        }
        // 文件名就是图像的标签
        size_t startPos = imagePath.find_last_of('/') + 1;
        size_t endPos = imagePath.find_last_of('-');
        size_t extensionPos = imagePath.find_last_of('.');
    ...
    if (dataMat.empty() || labelsMat.empty()) {
        std::cout << "Error: Empty data matrix or labels matrix. Unable to train." << std::endl;
        // 可以添加其他处理逻辑，例如返回错误代码或抛出异常
    } else {
        std::cout << "Dataset initialized successfully" << std::endl;
